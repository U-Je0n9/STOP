...
from .temporal_prompting import get_TemporalPrompt
...
def force_flush_debug_buffer(header="[DEBUG] force flush from caller"):
    _flush_debug_buffer(header=header)

def _safe_scalar_stat(x, fn_name):
    try:
        return getattr(x, fn_name)().item()
    except Exception:
        return None

def _summarize_tensor(x, print_rows=False):
    info = {
        "shape": tuple(x.shape),
        "dtype": str(x.dtype),
        "device": str(x.device),
...
def _format_debug_entry(name, info):
    msg = (
        f"[DEBUG] {name}: "
...
def _flush_debug_buffer(header=None):
    if header is not None:
        print(header)
    print("[DEBUG] ===== buffered tensor trace start =====")
    for line in DEBUG_BUFFER:
        print(line)
    print("[DEBUG] ===== buffered tensor trace end =====")

def _debug_tensor(name, x, print_rows=False):
    global DEBUG_TRIGGERED

    if x is None:
        msg = f"[DEBUG] {name}: None"
    ...
    with torch.no_grad():
        info = _summarize_tensor(x, print_rows=print_rows)
        msg = _format_debug_entry(name, info)
        DEBUG_BUFFER.append(msg)

        bad = info["nan"] or info["inf"]

        if bad and not DEBUG_TRIGGERED:
            DEBUG_TRIGGERED = True
            _flush_debug_buffer(header=f"[DEBUG] NaN/Inf first detected at {name}")
...
#             if finite_mask.any():
#                 finite_vals = x_float[finite_mask]
#                 print(f"[DEBUG] {name}: finite min={finite_vals.min().item():.6f}, "
#                       f"finite max={finite_vals.max().item():.6f}, "
#                       f"finite mean={finite_vals.mean().item():.6f}")
#             else:
#                 print(f"[DEBUG] {name}: no finite values")

#             if print_rows and x.dim() >= 2:
#                 bad_rows = torch.where(~torch.isfinite(x_float.view(x.shape[0], -1)).all(dim=1))[0]
#                 print(f"[DEBUG] {name}: bad row idx={bad_rows}")
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask=None, block_id=1, args=None):
        ...
        if args is not None:
            ...
            if getattr(args, "use_traj_prompt", 0):
                self.visual_prompt_length += 1
            if args.lora:
    ...
    def forward(self, x_tuple:tuple):
        ...
        if  visual:
            ...
            T = video_frame
            dim = x.size(-1)
            _debug_tensor("RAB.visual.input_x", x, print_rows=True)
            visual_prompt,frame_token= x[:self.visual_prompt_length,:,:],x[self.visual_prompt_length:,:,:].reshape(-1,BT,dim)
            _debug_tensor("RAB.visual.visual_prompt.before_ln", visual_prompt, print_rows=True)
            _debug_tensor("RAB.visual.frame_token.before_ln", frame_token, print_rows=True)
            frame_token = self.ln_1(frame_token)
            visual_prompt = self.ln_1(visual_prompt)
            _debug_tensor("RAB.visual.visual_prompt.after_ln", visual_prompt, print_rows=True)
            _debug_tensor("RAB.visual.frame_token.after_ln", frame_token, print_rows=True)
            ...
            for i in range(0,BT,B):
                key1[:,i:i+B, :] = torch.cat((
                            visual_prompt,
                            query1[:, i:i+B, :]), dim=0)    
            _debug_tensor("RAB.visual.query1", query1, print_rows=True)
            _debug_tensor("RAB.visual.key1", key1, print_rows=True)
            attention_output_frames = self.attention(query1,key1,key1) 
            _debug_tensor("RAB.visual.attention_output_frames.raw", attention_output_frames, print_rows=True)
            attention_output_frames = attention_output_frames.reshape(-1, B, dim)# [54*num_frames,batch_size, dim]
            ...
            _debug_tensor("RAB.visual.query2", query2, print_rows=True)
            _debug_tensor("RAB.visual.key2", key2, print_rows=True)
            attention_output_prompt = self.attention(query2,key2,key2)
            _debug_tensor("RAB.visual.attention_output_prompt", attention_output_prompt, print_rows=True)
            ...
            _debug_tensor("RAB.visual.output_x_after_attn", x, print_rows=True)
        ...
        if self.args.lora:
            ...
            lora_out = 0.1 * self.LoRA(x)
            _debug_tensor("RAB.visual.mlp_out", mlp_out, print_rows=True)
            _debug_tensor("RAB.visual.lora_out", lora_out, print_rows=True)
            x = x + mlp_out + lora_out
        else:
            # x = x + self.mlp(self.ln_2(x))
            mlp_out = self.mlp(self.ln_2(x))
            _debug_tensor("RAB.visual.mlp_out", mlp_out, print_rows=True)
            x = x + mlp_out
        _debug_tensor("RAB.visual.final_x", x, print_rows=True)
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.args = args
        if self.args.temporal_prompt in ['group1-1', 'group1-2', 'group2-1']:
            self.TemporalPrompt = get_TemporalPrompt(args=args)
        elif self.args.temporal_prompt in ['group2-2']:
            self.TemporalPrompt = get_TemporalPrompt(args=args)
        ...
        self.traj_prompt_length = 1 if getattr(args, "use_traj_prompt", 0) else 0       
        self.total_global_prompt_length = self.num_tokens + self.traj_prompt_length
        


 
        
    def incorporate_prompt(self, x, unified_visual_prompt, traj_prompt=None):
        ...
        if self.shared_latent_space == "transformer":
            ...
            # case 1: [B, 4, D] 기존 방식
            if unified_visual_prompt.size(1) == self.num_tokens:
                unified_visual_frame_prompt = (
                    unified_visual_prompt
                    .view(B, self.num_tokens, x.size(-1))
                    .unsqueeze(1)
                    .expand(-1, self.video_frames, -1, -1)
                )

            # case 2: [B, T*4, D] group2-2 inter-frame prompt가 더해진 방식
            elif unified_visual_prompt.size(1) == self.video_frames * self.num_tokens:
                unified_visual_frame_prompt = (
                    unified_visual_prompt
                    .view(B, self.video_frames, self.num_tokens, x.size(-1))
                )

            else:
                raise RuntimeError(
                    f"Bad unified_visual_prompt shape in incorporate_prompt: "
                    f"{unified_visual_prompt.shape}, "
                    f"expected second dim {self.num_tokens} or {self.video_frames * self.num_tokens}"
                )   
        else:
        ...
        if traj_prompt is not None:
            if traj_prompt.dim() != 3:
                raise RuntimeError(f"traj_prompt must be [B, 1, D], got {traj_prompt.shape}")

            if traj_prompt.size(0) != B or traj_prompt.size(1) != 1 or traj_prompt.size(2) != x.size(-1):
                raise RuntimeError(
                    f"Bad traj_prompt shape: {traj_prompt.shape}, "
                    f"expected [{B}, 1, {x.size(-1)}]"
                )

            traj_prompt = traj_prompt.to(
                device=unified_visual_global_prompt.device,
                dtype=unified_visual_global_prompt.dtype
            )
            ...
            with torch.no_grad():
                ...
                self.last_prompt_stats.update(
                    _norm_stats("cls_feat", cls_feat)
                )

                self.last_prompt_stats.update(
                    _norm_stats("traj_prompt", traj_prompt)
                )

                traj_norm = traj_prompt.detach().float().norm(dim=-1).mean()

                cls_norm = cls_feat.detach().float().norm(dim=-1).mean()

                self.last_prompt_stats["traj_over_cls"] = (
                    traj_norm / (cls_norm + 1e-6)
                ).item()

            unified_visual_global_prompt = torch.cat(
                [traj_prompt, unified_visual_global_prompt],
                dim=1
            )
            ...
            if i == 0:
                ...
                if i <= len(unified_visual_prompt):
                    if self.shared_latent_space == "transformer":         
                        ...
                        cur_prompt = unified_visual_prompt[i]

                        # case 1: [B, 4, D] 기존 방식
                        if cur_prompt.size(1) == self.num_tokens:
                            unified_visual_frame_prompt = (
                                cur_prompt
                                .view(B, self.num_tokens, x.size(-1))
                                .unsqueeze(1)
                        ...
                        # case 2: [B, T*4, D] group2-2 방식
                        elif cur_prompt.size(1) == self.video_frames * self.num_tokens:
                            unified_visual_frame_prompt = (
                                cur_prompt
                                .view(B, self.video_frames, self.num_tokens, x.size(-1))
                                .permute(2, 1, 0, 3)
                            ...
                            raise RuntimeError(
                                f"Bad unified_visual_prompt shape in forward_deep_prompt: "
                                f"{cur_prompt.shape}, "
                                f"expected second dim {self.num_tokens} or {self.video_frames * self.num_tokens}"
                            )
                    else:
                        raise NotImplementedError('Do not find implementation of {}'.format(self.shared_latent_space))
                    global_prompt_len = self.num_tokens
                    if getattr(self.args, "use_traj_prompt", 0):
                        global_prompt_len += 1

                    hidden_states_global = hidden_states[:global_prompt_len, :, :]
                    hidden_states = hidden_states[global_prompt_len:, :, :].reshape(
                        -1, self.video_frames, B, x.size(-1)
                    )   
    ...
    def forward(self, x: torch.Tensor,unified_visual_prompt, video_frame=-1):
        ...
        _debug_tensor("VT.input_x", x, print_rows=True)
        _debug_tensor("VT.input_unified_visual_prompt", unified_visual_prompt, print_rows=True)
        ...
        if self.args.temporal_prompt in ['group2-2']:
            ...
            _debug_tensor("VT.before_TemporalPrompt", x, print_rows=True)
            x = self.TemporalPrompt(x)
            _debug_tensor("VT.after_TemporalPrompt", x, print_rows=True)
        ...
        if self.linear_patch == '3d':
            ...
            x = self.conv1(x)
            _debug_tensor("VT.after_conv1", x, print_rows=True)
        # [B x T, width, grid x grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)
        # [B x T, grid x grid, width]
        x = x.permute(0, 2, 1)
        _debug_tensor("VT.after_patch_flatten", x, print_rows=True)
        ...
        _debug_tensor("VT.after_cls_concat", x, print_rows=True)
        #x = x + self.positional_embedding[1:,:].to(x.dtype)
        x = x + self.positional_embedding.to(x.dtype)
        _debug_tensor("VT.after_positional_embedding", x, print_rows=True)
        x = self.ln_pre(x)
        _debug_tensor("VT.after_ln_pre", x, print_rows=True)
        
        traj_prompt = None

        if self.args.temporal_prompt in ['group2-2']: 
            inter_frame_prompt = self.TemporalPrompt.get_inter_frame_prompt(x)
            _debug_tensor("VT.inter_frame_prompt", inter_frame_prompt, print_rows=True)

            if getattr(self.args, "use_traj_prompt", 0):
                traj_prompt = self.TemporalPrompt.get_trajectory_prompt(x)
                _debug_tensor("VT.traj_prompt", traj_prompt, print_rows=True)

            if (
                self.shared_latent_space == "linear"
                and unified_visual_prompt.size(2) == self.num_tokens
                and inter_frame_prompt.size(2) == self.video_frames * self.num_tokens
            ):
                unified_visual_prompt = (
                    unified_visual_prompt
                    .unsqueeze(3)
                    .expand(-1, -1, -1, self.video_frames, -1)
                    .permute(0, 1, 3, 2, 4)
                    .reshape(
                        unified_visual_prompt.size(0),
                        unified_visual_prompt.size(1),
                        self.video_frames * self.num_tokens,
                        unified_visual_prompt.size(-1)
                    )
                )
        ...
        x = self.incorporate_prompt(x, unified_visual_prompt[0], traj_prompt=traj_prompt)
        # if self.args.temporal_prompt in ['group2-2']: 
        #     inter_frame_prompt = self.TemporalPrompt.get_inter_frame_prompt(x)
        #     _debug_tensor("VT.inter_frame_prompt", inter_frame_prompt, print_rows=True)
        #     if (
        ...
        _debug_tensor("VT.after_incorporate_prompt", x, print_rows=True)
        x = x.permute(1, 0, 2)  					# NLD -> LND
        _debug_tensor("VT.before_forward_deep_prompt", x, print_rows=True)
        
        
        # org forward 
        #x = self.transformer(x, video_frame=video_frame, visual=True)
        x= self.forward_deep_prompt(x,unified_visual_prompt)
        _debug_tensor("VT.after_forward_deep_prompt", x, print_rows=True)
        x = x.permute(1, 0, 2)  					# LND -> NLD
        _debug_tensor("VT.final_output", x, print_rows=True)
        return x
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, unified_visual_prompt, return_hidden=False, video_frame=-1):
        ...
        _debug_tensor("CLIP.encode_image.input_image", image, print_rows=True)
        _debug_tensor("CLIP.encode_image.input_unified_visual_prompt", unified_visual_prompt, print_rows=True)

        hidden = self.visual(image.type(self.dtype), unified_visual_prompt, video_frame=video_frame)
        _debug_tensor("CLIP.encode_image.hidden_from_visual", hidden, print_rows=True)

        hidden = self.visual.ln_post(hidden) @ self.visual.proj
        _debug_tensor("CLIP.encode_image.after_proj", hidden, print_rows=True)

        # 기존 global prompt 개수 4에 trajectory prompt가 붙으면 5가 됨
        if hasattr(self.visual, "total_global_prompt_length"):
            global_prompt_len = self.visual.total_global_prompt_length
        else:
            global_prompt_len = self.visual.num_tokens

        x = hidden[:, 0, :]
        _debug_tensor("CLIP.encode_image.cls_x", x, print_rows=True)

        # 기존 hidden[:, 4:, :] → hidden[:, global_prompt_len:, :]
        frame_cls_token = hidden[:, global_prompt_len:, :].reshape(
            hidden.size(0),
            -1,
            self.video_frames,
            hidden.size(-1)
        )[:, 0, :, :]
        _debug_tensor("CLIP.encode_image.frame_cls_token", frame_cls_token, print_rows=True)

        # 기존 hidden[:, :4, :] → hidden[:, :global_prompt_len, :]
        global_prompt_feature = hidden[:, :global_prompt_len, :]

        # p_traj를 맨 앞에 붙였으므로 hidden[:, 0:1, :]는 trajectory prompt feature가 됨
        if getattr(self.args, "use_traj_prompt", 0):
            global_prompt_feature0 = hidden[:, 1:2, :]
        else:
            global_prompt_feature0 = hidden[:, 0:1, :]

        _debug_tensor("CLIP.encode_image.global_prompt_feature", global_prompt_feature, print_rows=True)
        _debug_tensor("CLIP.encode_image.global_prompt_feature0", global_prompt_feature0, print_rows=True)

        print(
            f"[DEBUG] global_visual_prompt_length={self.visual.num_tokens}, "
            f"total_global_prompt_length={global_prompt_len}, "
            f"hidden_shape={hidden.shape}"
        )
        ...
        if self.visual_output_type == "global_prompt0":
            ...
            _debug_tensor("CLIP.encode_image.output.average_global_prompt", output, print_rows=True)
            return output

        elif self.visual_output_type == "average_frame_cls_token":
            output = torch.mean(frame_cls_token, 1, False)
            _debug_tensor("CLIP.encode_image.output.average_frame_cls_token", output, print_rows=True)
            return output
            ...
            _debug_tensor("CLIP.encode_image.global_local_feature", global_local_feature, print_rows=True)
            output = torch.mean(global_local_feature, 1, False)
            _debug_tensor("CLIP.encode_image.output.average_global_prompt_and_frame_cls_token", output, print_rows=True)
            return output
            ...
            _debug_tensor("CLIP.encode_image.global0_local_feature", global0_local_feature, print_rows=True)
            return output

        else:
            raise NotImplementedError(
                'Do not find implementation of {}'.format(self.visual_output_type)
            )
